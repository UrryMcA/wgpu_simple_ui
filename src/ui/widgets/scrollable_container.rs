...
impl ScrollableContainer {
    ...
    pub fn set_scroll(&mut self, x: f32, y: f32) {
        ...
        let max_x = (child_size.width - self.viewport_size.width).max(0.0);
        let max_y = (child_size.height - self.viewport_size.height).max(0.0);
    ...
    /// Возвращает область, которая должна быть видна (clipping rectangle) в абсолютных координатах
    fn clip_rect(&self) -> Rect {
        Rect::new(
            self.position.x + self.margin.left,
            self.position.y + self.margin.top,
            (self.viewport_size.width - self.margin.left - self.margin.right).max(0.0),
...
impl RenderBox for ScrollableContainer {
    fn layout(&mut self, constraints: Constraints, ctx: &mut dyn LayoutContext) -> Size {
        if self.id.is_none() { /* регистрация (опционально) */ }

        // Вычитаем margin из доступного пространства
    ...
    fn set_position(&mut self, pos: Point) {
        self.position = pos;
        // Позиция ребёнка будет скорректирована в render с учётом скролла
    }
    ...
    fn render(&mut self, ctx: &mut RenderContext) {
        // 1. Вычисляем scissor-область (абсолютные координаты)
        let clip = self.clip_rect();
        if clip.w <= 0.0 || clip.h <= 0.0 {
            return; // Нечего показывать
        }

        // 2. Пересекаем с текущим scissor (если есть)
        let final_scissor = if let Some(parent_scissor) = ctx.current_scissor() {
            parent_scissor.intersect(&clip)
        } else {
            Some(clip)
        };

        if let Some(scissor) = final_scissor {
            ctx.push_scissor(scissor);
        }

        // 3. Сдвигаем ребёнка с учётом прокрутки
        let old_child_pos = self.child.position();
        let new_child_pos = Point::new(
            old_child_pos.x - self.scroll_x,
            old_child_pos.y - self.scroll_y,
        );
        self.child.set_position(new_child_pos);

        // 4. Рендерим ребёнка (он будет обрезан активным scissor)
        self.child.render(ctx);

        // 5. Восстанавливаем позицию ребёнка
        self.child.set_position(old_child_pos);

        // 6. Восстанавливаем scissor
        if final_scissor.is_some() {
            ctx.pop_scissor();
        }
    }
    ...
    fn handle_event(&mut self, event: &Event, _ui_manager: &mut UiManager) -> bool {
        ...
        // Прокрутка также может обрабатываться через handle_mouse_wheel
        false
    }
